...
| compilation | Compiler 每启动一次编译，就会生成一个 Compilation 对象用于追踪此次编译。 |
| make | 从 Entry 开始读取文件，根据文件类型和配置的 Loader 对文件进行编译，编译完后再找出该文件依赖的文件，递归的编译和解析。 |
| after-compile | 这次编译完成。 |
